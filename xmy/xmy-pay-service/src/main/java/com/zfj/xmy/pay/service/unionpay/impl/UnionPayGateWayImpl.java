...
@Service
public class UnionPayGateWayImpl implements UnionPayGateWay {

	
	private static Logger logger = LoggerFactory
			.getLogger(UnionPayGateWay.class);

	@Autowired
	private PayBase payBase;

	@Override
	public String gateWayConsume(String orderNum, Double amount,
			String subject, String attach, String notify_url,String frontUrl) {
		// 传入参数
		...
		// 配置的可变参数 - 测试
		UnionPayConfigVo unionPayConfig = payBase.getPayConfigObject(PayBaseImpl.TRADETYPE_UNIONPAY_GATEWAY, UnionPayConfigVo.class);
		...
		contentData.put("channelType", "07"); // 渠道类型 08手机
		...
		Map<String, String> reqData = AcpService.sign(contentData,
				PayConstant.PAY_CONSTANT.UNIONPAY_ENCODING);// 报文中certId,signature的值是在signData方法中获取并自动赋值的，只要证书配置正确即可。
		...
		String html = AcpService.createAutoFormHtml(requestFrontUrl, reqData,
				PayConstant.PAY_CONSTANT.UNIONPAY_ENCODING); // 生成自动跳转的Html表单
