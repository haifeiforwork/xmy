...
<table class="layui-table">
  ...
  <tbody>
      ...
      <td><fmt:formatDate value="${item.putwayTime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
      <td>
		    <div class="site-demo-button  " >
		      <button class="layui-btn" id="look" data-id="${item.id }" >查看</button>
		      <button class="layui-btn" id="edit" data-id="${item.id }" >编辑</button>
		      <button class="layui-btn" id="del" data-id="${item.id }"  >删除</button>
		      <button class="layui-btn" id="copy" data-id="${item.id }" >复制商品</button>
		    </div>
      </td>
    </tr>
  	</c:forEach>
...
layui.use(["pager","form","upload"],function(){
	...
	$("table tr td #edit").click(function(){
		var id = $(this).data("id");
		//var categoryName = $("#category").val(),brandName = $("#brandName").val(),supplierName = $("#supplierName").val(),keyword = $("#keyword").val();
		//window.location.href="goods/toUpdateGoods?id="+id+"&pageIndex="+pageIndex+"&ruleCategoryName="+categoryName+"&ruleBrandName="+brandName+"&ruleSupplierName"+supplierName+"&ruleKeyWord="+keyword;
		window.open("goods/toUpdateGoods?id="+id+"&pageIndex="+pageIndex);
		//window.location.href="goods/toUpdateGoods?id="+id+"&pageIndex="+pageIndex;
	})
	
	var xmyPcUrl = "${xmyPcUrl}";  //香满圆线上pc网址
	//查看
	$("table tr td #look").click(function(){
		var id = $(this).data("id")
		window.location.href=xmyPcUrl+"goods/"+id+"/0/0";
	});
	...
	//复制商品
	$("table tr td #copy").click(function (){
		$.ajax({
			url:"goods/copyGoods",
			type:"get",
