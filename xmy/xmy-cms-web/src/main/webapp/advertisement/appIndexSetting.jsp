...
//选中广告样式
function chooseAdvertisementStyle(style){
	alert(style);
}
...
layui.use(["form","upload","tool","layer"], function(){
	...
	$(document).on("click","#imgdiv",function(){
		_thisSrc = $(this).find("#imgsrc");
		_thisPath = $(this).find("#imgPath");
	})
		    ...
		    ,success: function(res){ //上传成功后的回调
		    	 var result = res.data;
			      ...
			      if(result.success == true){
			    	  _thisSrc.attr('src',result.filePath) ; 
			    	  _thisPath.val(result.filePath) ;
			    	  form.render();
			      }else{
	...
	form.on('radio(test)',function (data){
			if(data.value==1){
				$('#advertisementStyle2').addClass("advertisementStyle-none");
				$('#advertisementStyle1').removeClass("advertisementStyle-none");
			}else if(data.value==2){
				$('#advertisementStyle1').addClass("advertisementStyle-none");
				$('#advertisementStyle2').removeClass("advertisementStyle-none");
			}
	});
