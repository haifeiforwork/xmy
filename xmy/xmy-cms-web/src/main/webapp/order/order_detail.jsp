 	 	...
 	 	<div class="layui-body layui-tab-content site site-body site-custom-body layui-form-item">
		 	 ...
		 	 <blockquote class="layui-elem-quote">
				...
				<c:if test="${orderMap.order.isDel==1 }">
					<c:if test="${orderMap.order.returnStatus==0 || empty orderMap.order.returnStatus }">
						<button id="delete" class="layui-btn" >作废订单</button>
						<button id="user" class="layui-btn">客户清单</button>
						...
						<c:if test="${orderMap.order.shipStatus==2 }">
							<button id="updatePayStatus" class="layui-btn" >改为已支付</button>
						</c:if>
					...
					<c:if test="${orderMap.order.returnStatus==1 }">
						<button id="delete" class="layui-btn" >订单作废</button>
						<i class="layui-icon"  style="font-size: 16px; color: #c2c2c2;">●已退货</i>
					</c:if>
				</c:if>
			...
			<fieldset class="layui-elem-field site-demo-button">
	  			...
	  			<div style="width: 90%" >
					  ...
					  <c:forEach items="${orderMap.goods }" var="good">
					    <tr>
					      ...
					      <td><fmt:formatNumber value="${good.sumPrice }" pattern="#.00"/></td>
					     <%--  <c:if test="${orderMap.order.isDel==1 }">
			      			<td>
						  ...
						  <div style="width: 90%;">
						    ...
						    <form class="layui-form" action="order/updateOrderDate" >
						  		<div class="layui-form-item">
						  			<input type="text" value="<fmt:formatDate value="${orderMap.order.extendTime }" pattern="yyyy-MM-dd HH:mm:ss"/>" onclick="layui.laydate({elem: this, istime: true, format: 'YYYY-MM-DD hh:mm'})" style="width: 200px;float: left;margin-left: 40px" name="updateTime" placeholder="扩展时间" class="layui-input">
				...
				<c:if test="${orderMap.order.isDel==1 }">
					<c:if test="${orderMap.order.shipStatus==5 }">
						...
						<form class="layui-form" action="order/updateOrderDelivery" >
							...
							<div class="layui-input-block" style="width: 250px">
								<select name="deliveryId" id="sel" lay-verify="deliveryId">
								<option value="" class="op">请选择配送人</option>
								<c:forEach items="${sessionScope.deliveryUser }" var="data">
									<option value="${data.id }" data="${data.name }，联系方式：${data.mobilePhone }" class="op">${data.name }</option>
								</c:forEach>
								</select>
							</div>
							...
							<div class="layui-form-item layui-form-text">
								...
								<br><br><br>
								<input type="hidden" id="isSendSMS" name="isSendSMS" value="1">
							</form>
						</div>
						<div class="layui-form-item" style="background-color: #eeeeee;">
						<form class="layui-form" action="order/updateSanOrder" >
							<br>
							<label class="layui-form-label">选择快递：</label>
							<div class="layui-input-block" style="width: 250px">
								<select name="kdniaoExpCode" lay-verify="kdniaoExpCode">
								<option value="" class="op">请选择快递公司</option>
								<c:forEach items="${kdList }" var="kd">
									<option value="${kd.code }" class="op">${kd.name }</option>
								</c:forEach>
								</select>
							</div>
							<br><br>
							<label class="layui-form-label">物流单号：</label>
							 <div class="layui-input-block">
			    				 <input type="text" class="layui-input" style="width: 250px" value="" lay-verify="logisticsNo" placeholder="请输入物流单号" name="logisticsNo"/>
			   				 </div>
			   				 <br></br>
							<input type="text" id="or" style="display: none" name="ids" value="${orderMap.order.id }"/>
								<div class="layui-input-inline" style="margin-left: 20px">
				  ...
				  <div style="width: 90%" >
				    		...
				    		<li class="orderLi">身份证号：${orderMap.order.consigneeIdcard }</li><br>
				...
				<c:if test="${!empty orderMap.order.invoiceContent }">
					<fieldset class="layui-elem-field site-demo-button">
					  ...
					  <div style="width: 90%;">
					    ...
					    <li class="orderLi">公司名称：${orderMap.order.companyName }</li><br>
				...
				<fieldset class="layui-elem-field site-demo-button">
				  ...
				  <div style="width: 90%;">
				    ...
				    <table class="layui-table" style="margin-left: 40px" lay-skin="line">
  						...
  						<tr>
  							<td>优惠方式：<c:if test="${empty orderMap.order.businessRemark }">无</c:if> <c:if test="${!empty orderMap.order.businessRemark }">${orderMap.order.businessRemark }</c:if> </td>
  						...
  						<tr>
  							...
  							<td>支付方式：
					        	...
					        	<c:if test="${orderMap.order.payType==5 }">货到付款(${orderMap.order.total })</c:if>
					        	<c:if test="${orderMap.order.payType==6 }">混合支付</c:if></td>
  						</tr>
  						<tr>
  							<td>提货卡支付：
	  							<c:if test="${orderMap.order.shipStatus==2 || empty orderMap.order.pay }">0.00</c:if> 
	  							<c:if test="${orderMap.order.shipStatus>=3 || orderMap.order.shipStatus==1 }">${orderMap.order.pay }</c:if>
  							</td>
  							<td>第三方支付金额：
  								<c:if test="${orderMap.order.payType==5 }">
  									0.00
  								</c:if>
  								<c:if test="${orderMap.order.payType!=5 && orderMap.order.payType!=6}">
		  							<c:if test="${orderMap.order.shipStatus==2}">0.00</c:if>
		  							<c:if test="${orderMap.order.shipStatus>=3}">${orderMap.order.actual }</c:if>
  								</c:if>
  								<!-- 混合支付 -->
  								<c:if test="${orderMap.order.payType==6}">
		  							<c:if test="${orderMap.order.shipStatus==2}">0.00</c:if>
		  							<c:if test="${orderMap.order.shipStatus>=3}">
		  								${orderMap.order.actual }
		  								<c:forEach items="${tradeType }"  var="item">
		               			 	 		<c:if test="${item.key == orderMap.order.thirdPartyPayType }">(${item.value })</c:if> 
		               					</c:forEach>
		  							</c:if>
  								</c:if>
  							</td>
  						...
  						<tr>
  							<td><i data-method="offset" class="layui-icon" style="font-size: 15px; color: #009688;">
  							实付金额：￥
  							<c:if test="${orderMap.order.shipStatus==2 }">0.00</c:if> 
  							<c:if test="${orderMap.order.shipStatus>=3 && orderMap.order.payType != 5}">${orderMap.order.total }</c:if>
  							<c:if test="${orderMap.order.shipStatus==1 }">${orderMap.order.pay }</c:if>
  							<c:if test="${orderMap.order.shipStatus==1 && empty orderMap.order.pay }">0.00</c:if>
  							<c:if test="${orderMap.order.shipStatus>=3 && orderMap.order.payType == 5 }">${orderMap.order.pay }</c:if>
  							元</i></td>
  							<td></td>
  						</tr>
					</table>
