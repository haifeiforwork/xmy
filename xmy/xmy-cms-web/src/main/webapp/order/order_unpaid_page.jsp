      ...
      <tr>
        ...
        <td>
        	<c:if test="${data.isDel==1 }">
	        	<c:if test="${data.shipStatus==1 }"><i class="layui-icon" style="font-size: 12px; color: #009688;">●待审核</i> </c:if>
	        	<c:if test="${data.shipStatus==2 }"><i class="layui-icon" style="font-size: 12px; color: #009688;">●待支付</i> </c:if>
	        	<c:if test="${data.shipStatus==3 }">供货确认</c:if>
	        	<c:if test="${data.shipStatus==4 }">备货中</c:if>
	        	<c:if test="${data.shipStatus==5 }">待发货</c:if>
	        	<c:if test="${data.shipStatus==6 }"><i class="layui-icon" style="font-size: 12px; color: #FF5722;">●已发货</i></c:if>
	        	<c:if test="${data.shipStatus==7 }"><i class="layui-icon" style="font-size: 12px; color: #c2c2c2;">●已完成</i></c:if>
        	</c:if>
        ...
        <td>
        	<c:if test="${data.returnStatus==0 || empty data.returnStatus }">正常</c:if>
        	<c:if test="${data.returnStatus==1 }">已退货</c:if>
        	<c:if test="${data.returnStatus==2 }">退货中</c:if>
        </td>
        <td>
        <%-- <c:if test="${data.isDel==1 }">
		...
		<a href="order/detail/${data.id }"><button class="layui-btn">编辑</button></a>
        </td>
      </tr>
    </c:forEach>
  </tbody>
