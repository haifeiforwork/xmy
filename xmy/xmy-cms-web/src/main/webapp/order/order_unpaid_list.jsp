...
<div class="layui-layout layui-layout-admin">
  ...
  <div class="layui-tab layui-tab-brief layui-form" lay-filter="demoTitle">
  	...
  	<i class="layui-icon" style="font-size: 20px; color: #009688;">待支付</i>
 	 		<hr>
  		<div class="layui-tab-item layui-show">
			<form id="myForm">
				...
				<div class="layui-input-inline">
			      <select name="ship_status" lay-filter="ship_status" class="q-rule" zfj-query="ship_status:eq" disabled >
			        <option value="" <c:if test="${params.shipTtatus == 0}">selected="selected"</c:if>>订单状态</option>
			        <option value="1" <c:if test="${params.shipTtatus == 1}">selected="selected"</c:if>>待审核</option>
			        <option value="2" <c:if test="${params.shipTtatus == 2}">selected="selected"</c:if> selected="selected">代付款</option>
			        <option value="3" <c:if test="${params.shipTtatus == 3}">selected="selected"</c:if>>供货确认</option>
			        <option value="4" <c:if test="${params.shipTtatus == 4}">selected="selected"</c:if>>已供货</option>
			        <option value="5" <c:if test="${params.shipTtatus == 5}">selected="selected"</c:if>>已备货</option>
			        <option value="6" <c:if test="${params.shipTtatus == 6}">selected="selected"</c:if>>待发货</option>
			        <option value="7" <c:if test="${params.shipTtatus == 7}">selected="selected"</c:if>>已发货</option>
			      </select>
			    </div>
			    <div class="layui-input-inline">
			    ...
			    <div class="layui-input-inline">
			    	 <input type="text" class="q-rule layui-input" zfj-query="payment_name:cn" value="${params.payment_name}" placeholder="请输入用户账号" name="paymentName"/>
			    </div>
			    <div class="layui-input-inline">
  ...
  <script type="text/javascript" src="order/js/order_unpaid_list.js"></script>
</div>
</body>
</html>