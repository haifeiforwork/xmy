...
//			String qqopenid = QQLogin.qqTokenToGetOpenid(qqtoken);
			...
			System.out.println("获取不到 unionid-------"+qqunionid);
			if ("(null)".equals(qqunionid)) {
                throw new BusinessException("获取不到 unionid");
            }
			criteria.clearCriteria();
		...
		//2.1 更新redis
		redisTokenManager.setKey(user.getAppToken(),JSONUtils.toJSONString(user.getId()));
		return out;
	}
	...
	public void updateScancode(Long userId, String code) {
		...
		userMapper.updateByPrimaryKeySelective(user);
		redisTokenManager.setKey(SystemConstant.REDIS_SCANCODE_PREF + code,code,3600); //一小时过期
	}
	...
	public boolean validUserName(String name){
		...
		reqData.putValue("mobile_phone", name, SystemConstant.REQ_PARAMETER_EQ);
