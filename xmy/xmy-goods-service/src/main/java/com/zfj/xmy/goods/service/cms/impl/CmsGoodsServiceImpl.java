...
/*		String categoryId = goods.getCategoryId();
	...
	public int deleteGoods(String[] idArry){
		...
		for (String id : idArry) {
			...
			goods.setIsPutway(SystemConstant.GoodsConstant.NOT_PUTWAY);//回收商品商品默认下架
	...
	public Map<String,Object> ImgCorrelation(String  fileName,String targetPath,String ossPath){
	...
	public void getFileGoodsCode(){
		String codes = "";
		 Map<String,Object> result = new  HashMap<String,Object>();
		 String imgName = "";
		 File file = new File("C:/Users/Administrator/Desktop/没上传起的图片/01/");
		 File[] files = file.listFiles();
		 for (int i = 0; i < files.length; i++){
	...
	public Map<String,Object> goodsImageCorrelation(){
		 Map<String,Object> result = new  HashMap<String,Object>();
		 String imgName = "";
		 File file = new File("C:/Users/Administrator/Desktop/180207礼盒/01/");
		 File[] files = file.listFiles();
		 for (int i = 0; i < files.length; i++){
			GoodsWithBLOBs goods = new  GoodsWithBLOBs();
		 	imgName = files[i].getName();
            if (imgName.indexOf("-") > -1){
                ...
                List<GoodsWithBLOBs> list = findGoodsByCode(goodsCode); 
    			if (list.size() > 0) {
    				String dir = "http://img.xmy365.com/goods/goods/20180207/";
	 ...
	 * @author hexw
	 */
	public void batchAddMainImg(File file, GoodsWithBLOBs goods,String dir){
		GoodsImage goodsImage = new GoodsImage();
	 ...
	 * @author hexw
	 */
	public void batchAddDetailImg(File file, GoodsWithBLOBs goods,String dir){
	...
	public int copyGoods(Long  goodsId){
		...
		if (ObjectUtil.isNotNull(goods)) {
			...
			// 4. 保存商品
				...
				goods.setIsSearch(SystemConstant.GoodsConstant.NO);//复制商品默认不能被搜索
				goodsMapper.insertSelective(goods);
			// 5. 复制商品图片
				reqData.clearValue();
				reqData.putValue("goods_id", goodsId, SystemConstant.REQ_PARAMETER_EQ);
	...
	public void updateGoodsSupplierName(){
		ReqData reqData = new ReqData();
		reqData.putValue("brand_name", "", SystemConstant.REQ_PARAMETER_EQ);
		...
		for (GoodsWithBLOBs goods : list) {
			...
			if (!StringUtil.isEmpty(goodsName)) {
				if (goodsName.contains("]")) {
					String supplierName = goodsName.substring(0, (goodsName.lastIndexOf("]"))+1);
					supplierName = supplierName.replace("[", "");
					supplierName = supplierName.replace("]", "");
					reqData.clearValue(); 
					reqData.putValue("name", supplierName, SystemConstant.REQ_PARAMETER_EQ);
					reqData.putValue("vid", 3, SystemConstant.REQ_PARAMETER_EQ);
					List<TermData> termDatas = termDataMapper.selectByExample(ReqUtil.reqParameterToCriteriaParameter(reqData));
					if (termDatas.size()>0) {
						TermData termData = termDatas.get(0); 
			/*			goods.setSupplierId(termData.getId());
						goods.setSupplierName(termData.getName());*/
						goods.setBrandId(termData.getId());
						goods.setBrandName(termData.getName());
						goodsMapper.updateByPrimaryKeySelective(goods);
					} else {
						TermData termData = new TermData();
						termData.setName(supplierName);
						termData.setSn("10010");
						termData.setStatus(0);
						termData.setVid(3l);
						termData.setWeight(1);
		...
		System.out.println("供货商为空的商品："+list.size());
	}
