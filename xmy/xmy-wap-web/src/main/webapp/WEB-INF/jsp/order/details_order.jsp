...
<div class="content" <c:if test="${(not empty orderDto.cancelEndTime && not empty orderDto.nowTime &&  orderDto.cancelEndTime <= orderDto.nowTime) || orderDto.isDel == 0}">style="bottom:0px;"</c:if>>
    <div class="state-content">
        <div class="state-item clearfix">
            <div class="col-70">
                <p class="order-status">
                	<c:if test="${orderDto.isDel == 0 }">订单已取消</c:if>
                	<c:if test="${orderDto.isDel != 0 }">
	                	<c:if test="${orderDto.status ==1}">等待付款</c:if>
            ...
            <c:if test="${orderDto.isDel != 0 && orderDto.status ==1}">
				<p>待付款的72小时后自动取消订单</p>
			</c:if>
            </div>
    ...
    <div class="weui-cells ad-gutter">
    	<c:set var="goodsPrice" value="0.00"/>
            ...
            <div class="weui-cell__bd order-sentence">
                ...
                <p class="item-title2">
                	<c:if test="${not empty goodsDto.points && goodsDto.points != 0}">
			        	${goodsDto.points }积分
			        	<c:set var="goodsPoints" value="${goodsPoints + goodsDto.points*goodsDto.num }"/>
			        </c:if>
			        <c:if test="${empty goodsDto.points || goodsDto.points == 0}">
			        	...
			        	<c:set var="goodsPrice" value="${goodsPrice + goodsDto.price*goodsDto.num  }"/>
			        </c:if>
                </p>
        ...
        <div class="weui-form-preview">
            <div class="weui-form-preview__bd">
                <div class="weui-form-preview__item">
                    <label class="weui-form-preview__label">商品总金额(不含运费)</label>
                    <span class="weui-form-preview__value color-danger"><fmt:formatNumber maxFractionDigits="2" minFractionDigits="2">${goodsPrice}</fmt:formatNumber></span>
                </div>
                <div class="weui-form-preview__item">
                ...
                <div class="weui-form-preview__item">
                    <label class="weui-form-preview__label">活动优惠</label>
                    <span class="weui-form-preview__value color-danger">-¥<fmt:formatNumber value="${empty orderDto.discountPrice ? 0.00 : orderDto.discountPrice }" pattern="0.00"/></span>
                </div>
                <div class="weui-form-preview__item">
                    <label class="weui-form-preview__label">总税费</label>
                    <span class="weui-form-preview__value color-danger">¥0.00</span>
                </div>
                <div class="weui-form-preview__item">
                ...
                <c:if test="${goodsPoints > 0}">
	                <div class="weui-form-preview__item">
	                    <label class="weui-form-preview__label">使用积分</label>
	                    <span class="weui-form-preview__value color-danger"><fmt:formatNumber maxFractionDigits="0">${goodsPoints}</fmt:formatNumber></span>
	                </div>
                </c:if>
                <div class="weui-form-preview__item">
                    <label class="weui-form-preview__label">使用优惠券</label>
                    <span class="weui-form-preview__value color-danger">-¥<fmt:formatNumber value="${empty orderDto.couponValue ? 0.00 : orderDto.couponValue }" pattern="0.00"/></span>
                </div>
            </div>
            <div class="weui-form-preview__ft real-value" style="overflow:hidden;">
            	<div clss="real-value-left" style="float:left">已付款 ：<span class="color-danger">
            		￥	<c:if test="${orderDto.status == 1 }">0.00</c:if>
            			<c:if test="${orderDto.status != 1}">
            				<c:if test="${orderDto.status == 5 }" >
				                	${orderDto.total}
			                </c:if>
			                <c:if test="${orderDto.status != 5 }" >
			                	<c:if test="${orderDto.payType == 5 }" >
			                		<fmt:formatNumber maxFractionDigits="2" minFractionDigits="2">${empty orderDto.pay ? 0.00 :  orderDto.pay}</fmt:formatNumber>
			                	</c:if>
			                	<c:if test="${orderDto.payType != 5 }" >
			                		<fmt:formatNumber maxFractionDigits="2" minFractionDigits="2">${orderDto.total }</fmt:formatNumber>
			                	</c:if>
			                </c:if>
            			</c:if> 
            	</span></div>
            	<div class="real-value-right">应付款 : <span class="color-danger">
            			<small>¥</small>
            			<c:if test="${orderDto.status == 1 }">
            				<span class="ad-size"><fmt:formatNumber maxFractionDigits="2" minFractionDigits="2">${orderDto.total }</fmt:formatNumber></span>
            			</c:if>
            			<c:if test="${orderDto.status != 1}">
            				<c:if test="${orderDto.status == 5 }" >
			                ...
			                <c:if test="${orderDto.status != 5 }" >
			                	<c:if test="${orderDto.payType == 5 }" >
			                		<span class="ad-size"><fmt:formatNumber maxFractionDigits="2" minFractionDigits="2">${empty orderDto.actual ? 0.00 :  orderDto.actual}</fmt:formatNumber></span>
			                	</c:if>
			                	<c:if test="${orderDto.payType != 5 }" >
			                		 <span class="ad-size"><fmt:formatNumber maxFractionDigits="2" minFractionDigits="2">0.00</fmt:formatNumber></span>
			                	</c:if>
			                </c:if>
            			</c:if>
            ...
            <div class="weui-form-preview__bd">
                ...
                <div class="weui-form-preview__item">
                    ...
                    <span class="weui-form-preview__value color-66">
                    	<c:if test="${empty orderDto.sendTime || orderDto.status <= 2 }">无</c:if>
                    	<c:if test="${not empty orderDto.sendTime && orderDto.status > 2 }">
...
<c:if test="${(empty orderDto.cancelEndTime || empty orderDto.nowTime ||  orderDto.cancelEndTime > orderDto.nowTime) && orderDto.isDel == 1}">
 <footer>
    	 	<c:if test="${orderDto.status == 1 }">
            ...
            <c:if test="${orderDto.status == 2 && orderDto.shipStatus < 5 && orderDto.cancelEndTime > orderDto.nowTime}">
            	<div class="weui-form-preview__ft btn-wrap">
			...
			<c:if test="${orderDto.status==4 }">
				<div class="weui-form-preview__ft btn-wrap" style="position:relative;">
