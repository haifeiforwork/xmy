...
<html>
	...
	<div class="content">
    	...
    	<div class="flexslider" style="box-shadow:none;">
    	  	<c:if test="${activityType==2&&activityId==36}"><!-- 天天特价 -->
    			<div class="active-sign" style="top: 0px;"><img src="resources/common/images/img-tttj@2x.png"/></div> 
    		</c:if>
    		<c:if test="${activityType==2&&activityId==35}"><!-- 冰点价 -->
    	 		<div class="active-sign active-sign-ice" style="padding-top: 5px;"><p><fmt:formatDate value="${data.startTime }" pattern="MM月dd日"/><p>12:00</p></p></div>
    	 	</c:if>
    	 	 <c:if test="${activityType==0 && not empty activityId}"><!-- 买即赠 -->
	        	<div class="active-sign" style="top: 0px;"><img src="resources/activity/images/rob-song.png"/></div>
	        </c:if>
	         <c:if test="${activityType==2&&activityId==38}"><!-- 每周特价-->
	        	<div class="active-sign active-sign-ice" style="padding-top: 10px;"><p>每周特价</p></div>
	        </c:if>
	        <c:if test="${activityType==2&&activityId==37}"><!-- 整件惠-->
	        	<div class="active-sign active-sign-ice" style="padding-top: 10px;"><p>整件惠</p></div>
	        </c:if>
            <ul class="slides">
      ...
      <!-- activityId=5-7&activityType=0买即赠   activityId=36&activityType=2天天特价   activityId=38&activityType=2 每周特价   activityId=35&activityType=2冰点价   activityId=37&activityType=2整件惠 -->
        ...
        <c:if test="${activityType==0 && not empty activityId}"><!-- 买即赠 -->
        	<div class="finish-time time-esc3"></div>
        	<input type="hidden" class="buyAndPresent-endTime" value='<fmt:formatDate value="${goods.endTime }" pattern="yyyy-MM-dd HH:mm:ss" />'>
        </c:if>
        <c:if test="${activityType==2&&activityId==36}"><!-- 天天特价 -->
        ...
        <div class="head-top">
            ...
            <div class="line">
                ...
                <c:if test="${empty pointsId }">
	                <c:if test="${!empty goods.activityPrice }"><fmt:formatNumber value="${goods.activityPrice - 0.4 }" pattern="#"/></c:if>
					<c:if test="${ empty goods.activityPrice }"><fmt:formatNumber value="${goods.price - 0.4}" pattern="#"/></c:if>
                </c:if>
                <c:if test="${!empty pointsId }">
                	0
                </c:if>
        ...
        <div class="message">
            ...
            <div class="norms">
                ...
                <div class="ly-80 lump">
                    <p class="carry">
                    	<span class="lump-info lump-infoac">商品：<a>${goods.name }</a></span>
                    	<span class="lump-info lump-infoac other-info" style="display: none">商家：<a>[${fn:substring(fn:substringBefore(goods.name,"]"), 1, 9) }]</a></span>
                    	<span class="lump-info lump-infoac other-info" style="display: none">包装方式：<a>${empty goods.packMethod ? '无' : goods.packMethod}</a></span>
                    	<span class="lump-info lump-infoac other-info" style="display: none">储存方式：<a>${empty goods.storeageMethod ? '无' : goods.storeageMethod }</a></span>
                    	<span class="lump-info lump-infoac other-info" style="display: none">重量/规格：<a>${empty goods.standard ? '无' : goods.standard }</a></span>
                    </p>
            ...
            <div class="weui-cells list">
                <a class="weui-cell weui-cell_access" href="comment/${goods.goodsId }" >
                    ...
                    <span><fmt:formatNumber value="${goods.goodsComment}" pattern="#"/>%</span>好评
                    </div>
                </a>
               <!-- 2017-11-2-->
    ...
    <div id="imgword-info" class="weui-popup__container">
        ...
        <div class="weui-popup__modal">
            ...
            <div class="modal-bd" style="overflow-x: hidden;overflow-y:auto; ">
                <div class="txtarea">
                    <!-- <img src="resources/common/images/t-head1.jpg" alt=""> -->
                    ${goods.phIntroduced }
                    <img src="${goods.categoryDescriptionImg }" alt="" style="width:83%;display:block;margin:0 auto;"> 
                    <div class="goods-infos-img"><img src="resources/common/images/img-sdys@2x.png" alt=""></div>
                </div>
