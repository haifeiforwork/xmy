...
public class CommonPayOrderServiceImpl extends BaseService implements CommonPayOrderService {
	...
	@Override
	@Transactional
	public void updateGoodsPutway() {
		Map<String, Integer> chekFeeFerightGodos = chekFeeFerightGodos();
		Long[] feeFreigthGoods = SystemConstant.NEWACIVITY.IS_TRUE_SUB;
		for (Long long1 : feeFreigthGoods) {
			//获取当前商品的销售数量
			Integer integer = chekFeeFerightGodos.get(long1.toString());
			//销售量大于或等于了100就下架这件商品
			if(integer>=100){
				Goods goods = goodsMapper.selectByPrimaryKey(long1);
				goods.setIsPutway(SystemConstant.GoodsConstant.NOT_PUTWAY);
