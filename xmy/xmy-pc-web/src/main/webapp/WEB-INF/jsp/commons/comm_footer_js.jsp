<%@ page language="java" pageEncoding="UTF-8" errorPage="/WEB-INF/jsp/commons/error/500.jsp"%>
<script type="text/javascript" src="resource/commons/js/jquery-1.9.1.min.js"></script>
<script type="text/javascript" src="resource/commons/js/public.js"></script>
<%-- 弹窗的js文件 --%>
<script type="text/javascript" src="resource/commons/js/easydialog.min.js"></script>
<%--分页插件js文件 --%>
<script type="text/javascript" src="resource/commons/js/laypage.js"></script>
<![if lt IE 9]>
  <script src="https://g.alicdn.com/aliww/ww/json/json.js" charset="utf-8"></script>
<![endif]>
<%-- 客服通讯的js --%>
<script src="https://g.alicdn.com/aliww/ww/json/json.js" charset="utf-8"></script>
<script src="https://g.alicdn.com/aliww/??h5.imsdk/4.0.1/scripts/yw/wsdk.js,h5.openim.kit/0.5.0/scripts/kit.js" charset="utf-8"></script>
<!-- 搜索js -->
<script type="text/javascript" src="resource/js/common/goods_search.js"></script>
