...
import com.zfj.xmy.order.persistence.app.pojo.dto.AppOutOrder;
...
public class OrderController {
	...
	//	Long userId = AppLocalInfo.getUserId(); 
		for (AppGoodsVo goods : list) {
			boolean activityGoods = onLineActivityService.judgeLuckWap(goods.getId());
			if(activityGoods && AppLocalInfo.getUserId() == null){
				throw new BusinessException("请登录后，购买活动商品");
			}
		}
...
//		cancelOrderIn.setUserid(userid);
	...
	public ResponseEntity<AppResp> buyNowAnonymous(@RequestBody List<ShoppingCart> shoppingCarts){
		List<AppGoodsVo>  list = new ArrayList<AppGoodsVo>();
		for (ShoppingCart cart : shoppingCarts) {
			boolean activityGoods = onLineActivityService.judgeLuckWap(cart.getGoodsId());
			if(activityGoods) throw new BusinessException("请登录后，购买活动商品");
			list.add(appOrderService.createAppGoodsVo(cart));
		}
		return AppResp.get(appOrderService.findOrder(list, null));
	}
	 ...
	 * @return ResponseEntity<AppResp>
	 * @author lij
	 * @date 2018年1月24日 下午2:28:00
	 */
	@RequestMapping(value="/getOrder",method=RequestMethod.POST)
	@Exclusion
	public ResponseEntity<AppResp> getOrder(@RequestBody Order orderNo){
		List<Order> orderByOrderNo = appOrderService.getOrderByOrderNo(orderNo.getNo());
		if(ObjectUtils.isEmpty(orderByOrderNo)){
			throw new BusinessException("未查询到订单信息，请核实订单编号是否有误 !");
		}else{
			AppOutOrder outOrder = new AppOutOrder();
			Order order = orderByOrderNo.get(0);
			outOrder.setConsigneeAddress(order.getConsigneeAddress());
			outOrder.setConsigneeName(order.getConsigneeName());
			outOrder.setConsigneePhone(order.getConsigneePhone());
			outOrder.setNo(order.getNo());
			outOrder.setPayMoney(order.getTotal().subtract(order.getPay()));
			return AppResp.get(outOrder);
		}
	}
