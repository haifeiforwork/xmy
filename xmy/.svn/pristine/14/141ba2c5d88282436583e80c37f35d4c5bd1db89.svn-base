...
public class CouponServiceImpl extends BaseService implements CouponService {
	...
	public int insertCoupon(Coupon coupon) {
		...
		generateMoreCoupons(couponid, count);
		return insert;
	}
	
	@Override
	@Transactional(rollbackFor=Exception.class)
	public void appendMoreCoupon(Long couponid, Long count) {
		if (ObjectUtil.isNull(couponid)) {
			throw new BusinessException("请选择优惠劵");
		}
        if (ObjectUtil.isNull(count)) {
        	throw new BusinessException("请输入要追加的数量");
		}
		//1.检查优惠劵类型是否存在
		Coupon coupon = couponMapper.selectByPrimaryKey(couponid);
		if (ObjectUtil.isNull(coupon)) {
			throw new BusinessException("优惠劵类别不存在");
		}
		//2.追加优惠劵记录
		generateMoreCoupons(couponid, count);
	}
	 ...
	 * @param count
	 * @Description 
	 * @date 2018年2月2日  上午10:23:15
	 * @author wy
	 * 2018
	 * @return void
	 */
	private void generateMoreCoupons(Long couponid, Long count){
	...
	public void findCouponsAndPage(ReqData reqData, PageBean pageBean) {
		...
		List<Coupon> coupons = couponUserExMapper.findCouponsAndPage(ReqUtil.reqParameterToCriteriaParameter(reqData), pageBean.getRowBounds());
