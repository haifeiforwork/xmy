...
public class PcCategoryServiceImpl extends BaseService  implements PcCategoryService {
	...
	public PcCategoryDir findCategoryMenu(long parentId){
		...
		if (list.size() > 0) {
			for (TermData childTermData : list) {
				...
				wordSegreqData.setSortname("seq");
				wordSegreqData.setSortorder("asc");
					...
					for (Object containerId : containerIds) {
						...
						if (ObjectUtil.isNotNull(container)) {
							...
							if (findAdImageByAdId(container.getAdId()).size() > 0){
								...
								//广告分类名称
								pcContainerDto.setSlogan(findAdTwoTypeName(container.getAdId()));
							}
							// 5. 查询货柜商品
							reqData.clearValue();
	 ...
	 * @return String
	 * @author lij
	 * @date 2018年1月30日 上午10:17:39
	 */
	public String findAdTwoTypeName(long id){
		AdInfo adInfo = adInfoMapper.selectByPrimaryKey(id);
		TermData termData = null;
		String str = "";
		if(!ObjectUtils.isEmpty(adInfo)){
			termData = termDataMapper.selectByPrimaryKey(adInfo.getCategoryId());
		}
	...
	public List<Goods> findChangeGoods(Long id, Integer pageIndex) {
		...
		for(int i=0;list.size()<4;i++){
			int index = new Random().nextInt(termdata.size());
			reqData.putValue("category_name", ","+termdata.get(index).getName()+",", SystemConstant.REQ_PARAMETER_EQ);
