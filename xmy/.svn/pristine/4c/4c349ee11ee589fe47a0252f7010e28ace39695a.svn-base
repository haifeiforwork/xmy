...
public class PcGoodsServiceImpl extends BaseService implements PcGoodsService {
	...
	public PcGoodsDto getGoodsDetail(long goodsId,Integer typeId,Long activityId) {
		...
		if(typeId!=-1 && activityId != 0){
			...
			boolean isLimitActivityGoods = commonLimitActivityService.checkGoodsIsLimitActivityGoods(goodsId); //判断是否是限时限量商品
			if (isLimitActivityGoods) {
	...
	public AdImage getGalleryImage(){
		...
		ReqData reqData = new ReqData();
		reqData.putValue("name", "pc首页-下拉大图", SystemConstant.REQ_PARAMETER_EQ);
