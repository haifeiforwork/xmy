...
public class LimitActivityServiceImpl extends BaseService implements LimitActivityService {
	 ...
	 */
	@Override
	public Integer isSelaGoodsByGoodsId(Long goodsId) {
		//目前只有写死了冰点价商品
		int i = 0;//商品可以购买
		ReqData reqData = new ReqData();
		reqData.putValue("limit_id", 35, SystemConstant.REQ_PARAMETER_EQ);
		reqData.putValue("goods_id", goodsId, SystemConstant.REQ_PARAMETER_EQ);
		List<LimitGoods> limitGoods = limitGoodsMapper.selectByExample(ReqUtil.reqParameterToCriteriaParameter(reqData));
		if(!ObjectUtils.isEmpty(limitGoods)){
			LimitGoods limitGoods2 = limitGoods.get(0);
			if(limitGoods2.getAllNum()<=limitGoods2.getCompleteNum()){
				i=2;//商品已经超过限购数量
			}
		}else{
			i=1;//商品不存在在冰点价中
