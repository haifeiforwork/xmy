...
<body ontouchstart>
	<header>
		<div class="down-app clearfix" style="display: block;">
			...
			<div class="down-app-a">
				<a href="javascript:;" class="download">立即打开</a>
			</div>
		</div>
	...
	<div id="totop"><img src="resources/common/images/totop.png" /></div>
		...
		<div class="container-wrap container-white">
			...
			<div class="row2 clearfix">
				<c:forEach items="${limitActivityDir[0].goodsList }" begin="0"
					end="5" var="limitGoods">
					<fmt:formatDate value="${limitGoods.beginTime }" var="beginDate"
						pattern="M月d日" />
					<c:choose>
						<c:when
							test="${limitGoods.completeNum < limitGoods.allNum && ((beginDate == curr && currMill >= 43200000) || (beginDate == yesterday && currMill < 43200000)) }">
							<div class="col-33 goods-activity-div"
								goods-id="${limitGoods.goodsId }"
								activity-id="${limitActivityDir[0].id }">
								...
								<p class="price">¥
								    <fmt:formatNumber value="${limitGoods.activityPrice }" pattern="#.00"/>
								</p>
							</div>
						</c:when>
						<c:otherwise>
							<div class="col-33 goods-div-gray">
								<c:if test="${limitGoods.completeNum >= limitGoods.allNum }">
									<img src="resources/common/images/buy-none.png" alt="" style="position:absolute;left:10%;top:15%;width:80%;z-index:666"/>
								</c:if>
			...
			<div class="list-wrap">
				...
				<div class="row2 clearfix row3">
					...
					<div class="clearfix shop-list row3">
						<c:if test="${fn:length(container.goods) >= 6 }">
							<c:forEach items="${container.goods }" var="goods" begin="0"
							end="${(fn:length(container.goods) - 1 - (fn:length(container.goods) - 1)%5)}"	 step="${(fn:length(container.goods) - 1 - (fn:length(container.goods) - 1)%5)/5 }">
								<div class="col-33 goods-div" goods-id="${goods.id }">
									...
									<p class="price">¥<fmt:formatNumber value="${goods.phonePrice }" pattern="#.00"/></p>
								</div>
							</c:forEach>
						</c:if>
						<c:if test="${fn:length(container.goods) < 6 }">
							<c:forEach items="${container.goods }" var="goods" begin="0" end="5">
								<div class="col-33 goods-div" goods-id="${goods.id }">
									<img
										src="${goods.imgPath }?x-oss-process=image/resize,m_fixed,h_123,w_123"
										alt="" class="shop-img">
		...
		<div class="newcomer-packet">
			<div class="packet-content">
				...
				<div class="packet-img">
					<img  class="home_img" url="${online.imgUrl }" />
				</div>
				<div class="packet-btn">
					<a href="javascript:;" class="packet-down-btn download"> 
					</a> <a href="javascript:;" class="packet-look-btn">
					</a>
				</div>
			</div>
		</div>
