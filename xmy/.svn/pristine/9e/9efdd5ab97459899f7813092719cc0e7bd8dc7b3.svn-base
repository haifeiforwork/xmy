...
<div class="layui-layout layui-layout-admin">
 ...
 <div class="layui-tab layui-tab-brief layui-form" >
 	<div class="layui-body layui-tab-content site site-body site-custom-body layui-form-item">
 		<form class="layui-form"  action="advertisement/updateAdvertisement" method="post" id="form">
 		<blockquote class="layui-elem-quote">广告基本信息</blockquote>
 				<div class="layui-form-item" style="width: 400px;">
				    <label class="layui-form-label">广告名称</label>
				    <div class="layui-input-block">
				      <input type="text" name="name"  lay-verify="name" value="${data.name }" autocomplete="off"  class="layui-input"  >
				    </div>
				</div>
				...
				<div class="layui-form-item">
				    <label class="layui-form-label">广告位置选择<i style="color:#F00">*</i></label>
				  	<input type="hidden" name="positionId" id="positionId" value="${data.positionId }">
				    <div class="layui-input-inline" style="width: 200px;">
				      <select lay-filter="positionName">
				        <option value="请选择"></option>
				        <c:forEach items="${positionData }" var="item">
				        	<option class="adPosition" value="${item.id }" ${item.id == data.positionId? 'selected':'' } >${item.name } </option>
				        </c:forEach>
				      </select>
				 	</div>
		  		</div>
		  		...
		  		<div class="layui-form-item">
				    ...
				    <div class="layui-input-block">
				      <input type="checkbox"  lay-skin="switch"  lay-filter="isCategory" class="isCategory" value="1" ${data.isCategory == '0'? 'checked':'' }  lay-text="ON|OFF" >
				      <input type="hidden" name="isCategory"  class="isCategory" value="${data.isCategory }" >
				    </div>
				</div>
				...
				<div class="layui-form-item" style="width: 400px;">
				    ...
				    <div class="layui-input-block">
				      <input type="text" id="categoryName" lay-verify="categoryName" value="${data.categoryName }" autocomplete="off"  class="layui-input" readonly="readonly" >
 		...
 		<div id="adImage">
 		 	<c:if test="${not empty imageData }">
 				<c:forEach items="${imageData }" var="item">
 					<div id="initImageData">
	 					<div class="layui-form-item" >
							 <div class="layui-inline uploadImg" >
							      <label class="layui-form-label"> 位置 ：${data.positionName }</label>
							      <div class="layui-input-inline">
							         	<input type="text" name="imgPath" id="imgPath" value="${item.imgPath }" class="layui-input" placeholder="点击上传图片"  readonly="readonly">
			 						 	<input type="hidden" name="imgUrl" id="imgUrl" value="${item.data }" > 
										<input type="hidden" name="imgWeight" id="imgWeight" value="${item.weight }">
										<input type="hidden" name="imgType" id="type" value="${item.type }">
							      </div>
							      <div class="layui-inline ">
									<input type="button" class="layui-btn layui-btn-danger delImg"  value="删除">
								  </div>
								  <div class="layui-inline " >
									<img alt="" style="width: 50px;height: 50px;" src="${item.imgPath }">
								  </div>
			   				</div> 
			   			</div>	
		   			</div>
...
layui.use(["form","upload","tool","layer"], function(){
	...
	var imgNum = 0 , positionName = '' , initPositionId = "${data.positionId}" ;  //最开始保存的广告位置id
	...
	var initImageData = $("#initImageData").html();
	...
	function adImage(){
		var html = '';
		var checkedPositionId =  $("#positionId").val();
		if (checkedPositionId == initPositionId ){ //选的之前保存的 
			$("#adImage").html(initImageData) ;
		} else {
